...
export default function App() {
  ...
  const completeQuest = (id: string) => {
    ...
    setPlayer(prev => ({
      ...
      totalGoldEarned: prev.totalGoldEarned + quest.gold,
  ...
  const buyItem = (itemId: string) => {
    ...
    const alreadyOwned = player.ownedItems.includes(itemId)
    setPlayer(prev => ({
      ...prev,
      gold: prev.gold - item.price,
      ownedItems: alreadyOwned ? prev.ownedItems : [...prev.ownedItems, itemId],
      equippedItems: alreadyOwned ? prev.equippedItems : [...prev.equippedItems, itemId],
    }))
  }
  ...
  return (
    <div className="app">
      <Header
        gold={player.gold}
        completedCount={player.completedQuests.length}
      ...
      >
        ...
        {player.equippedItems.length > 0 && (
          <span style={{ opacity: 0.6, fontSize: '0.8em' }}>
            · {player.equippedItems.length} 件装备
          </span>
        )}
      </button>
